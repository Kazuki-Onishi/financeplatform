...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    try {
      ...
      console.warn("Invalid ID token for lock", error);
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }
    ...
    if (!flags.includes("perm.lock")) {
      return NextResponse.json({ error: "Missing lock permission" }, { status: 403 });
    }
    ...
    try {
      await adminDb.runTransaction(async (tx) => {
        ...
        if (!["confirmed", "reviewed"].includes(receipt.status)) {
          throw new HttpError("Receipt is not in a lockable state", 409);
        }

        tx.update(receiptRef, {
          status: "locked",
          lockedBy: decoded.uid,
    ...
    return NextResponse.json(
      {
        ...
        lockedBy: updated?.lockedBy ?? decoded.uid ?? null,
        lockedAt: lockedAtIso,
      },
      { status: 200 },
    );
  } catch (error) {
    console.error("Unexpected lock error", error);
