...
const MEMBER_STATUS_OPTIONS: Array<{ value: "active" | "pending"; label: string }> = [
...
export function MembersPanel({ canManage, storeId, storeName, pushToast }: MembersPanelProps): JSX.Element {
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(payload?.members) ? (payload.members as MemberRow[]) : [];
      setMembers(list);
      setMembersError(null);
  ...
  const loadInvites = useCallback(async () => {
    ...
    try {
      ...
      const inviteList = Array.isArray(payload?.invites) ? (payload.invites as InviteRow[]) : [];
  ...
  useEffect(() => {
    ...
    loadMembers().catch((error) => console.error("[settings] member load error", error));
  }, [storeId, loadMembers]);

  useEffect(() => {
    if (effectiveCanManage) {
      loadInvites().catch((error) => console.error("[settings] invite load error", error));
    } else {
      setInvites([]);
      setInvitesError(null);
    }
  ...
  const toggleInviteFlag = useCallback((flag: PermissionFlag) => {
    setInviteFlags((prev) => (prev.includes(flag) ? prev.filter((entry) => entry !== flag) : [...prev, flag]));
  }, []);
  ...
  const handleCreateInvite = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      if (!effectiveCanManage) {
        pushToast("error", "You do not have permission to create invites for this store.");
        return;
      }
      const user = auth.currentUser;
      if (!user) {
    ...
    [effectiveCanManage, expiresAt, inviteFlags, loadInvites, maxUses, note, pushToast, role, storeId],
  );
  ...
  return (
    <section className="flex flex-col gap-6 rounded border border-neutral-200 bg-white p-4">
      ...
      <div className="space-y-2">
        {membersError ? (
          <div className="rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{membersError}</div>
        ) : null}
        <div className="overflow-x-auto rounded border border-neutral-200">
          <table className="min-w-full divide-y divide-neutral-200 text-sm">
            <thead className="bg-neutral-50">
              <tr>
                <th className="px-3 py-2 text-left font-medium text-neutral-600">Member</th>
                ...
                <th className="px-3 py-2 text-left font-medium text-neutral-600">Resigned</th>
                <th className="px-3 py-2 text-left font-medium text-neutral-600">Joined</th>
                {effectiveCanManage ? <th className="px-3 py-2 text-left font-medium text-neutral-600">Actions</th> : null}
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-200">
              {membersLoading ? (
                <tr>
                  <td className="px-3 py-4 text-center text-neutral-500" colSpan={effectiveCanManage ? 7 : 6}>
                    Loading members...
                  </td>
                </tr>
              ) : null}
              ...
              {members.map((member) => (
                <tr key={member.id} className="hover:bg-neutral-50/80">
                  <td className="px-3 py-2">
                    <div className="text-sm font-medium text-neutral-800">{member.displayName ?? "Unknown user"}</div>
                    <div className="text-xs text-neutral-500">{member.email ?? "?"}</div>
                    <div className="text-xs font-mono text-neutral-400">{member.id}</div>
                  </td>
                  <td className="px-3 py-2 text-neutral-700">{MEMBER_ROLE_LABELS[member.role]}</td>
                  <td className="px-3 py-2 text-neutral-500">{member.flags.length ? member.flags.join(", ") : "-"}</td>
                  <td className="px-3 py-2 text-neutral-500">{member.status}</td>
                  <td className="px-3 py-2 text-neutral-500">{member.resigned ? "Yes" : "No"}</td>
                  <td className="px-3 py-2 text-neutral-500">{formatIsoDatetime(member.joinedAt)}</td>
                  {effectiveCanManage ? (
                    <td className="px-3 py-2">
                      <button
                        type="button"
      ...
      {effectiveCanManage ? (
        <form className="flex flex-col gap-3 rounded border border-neutral-200 bg-neutral-50 p-4" onSubmit={handleMemberSave}>
          <h3 className="text-sm font-semibold text-neutral-700">Update member</h3>
          {!editingMemberId || !memberForm ? (
            <p className="text-xs text-neutral-500">Choose a member from the table to edit their role, flags, or status.</p>
          ) : (
            <>
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="flex flex-col gap-1 text-sm" htmlFor="member-edit-role">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setMemberForm((prev) =>
                        prev ? { ...prev, role: event.target.value as StoreMemberRole } : prev,
                      )
                    }
                ...
                <label className="flex flex-col gap-1 text-sm" htmlFor="member-edit-status">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setMemberForm((prev) =>
                        prev ? { ...prev, status: event.target.value as "active" | "pending" } : prev,
                      )
                    }
                    className="rounded border border-neutral-300 px-3 py-2"
                    disabled={savingMember}
              ...
              <fieldset className="flex flex-col gap-2 text-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {inviteFlagOptions.map((option) => {
                    ...
                    return (
                      <label key={option.value} className="flex items-center gap-2 text-xs text-neutral-600">
                        <input
                          type="checkbox"
                          checked={checked}
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(event) =>
                    setMemberForm((prev) => (prev ? { ...prev, resigned: event.target.checked } : prev))
                  }
                  disabled={savingMember}
                  className="rounded border-neutral-300"
                />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {effectiveCanManage ? (
          <form className="flex flex-col gap-3 rounded border border-neutral-200 p-4" onSubmit={handleCreateInvite}>
            <h3 className="text-sm font-semibold text-neutral-700">Create invite</h3>
            {invitesError ? (
              <div className="rounded border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{invitesError}</div>
            ) : null}
            <label className="flex flex-col gap-1 text-sm" htmlFor="members-invite-role">
              <span className="font-medium text-neutral-600">Role</span>
              <select
            ...
            <label className="flex flex-col gap-1 text-sm" htmlFor="members-invite-max">
              ...
              <input
                ...
                disabled={creatingInvite}
              />
              <span className="text-xs text-neutral-400">Use 0 for unlimited.</span>
            </label>

            <label className="flex flex-col gap-1 text-sm" htmlFor="members-invite-expires">
              <span className="font-medium text-neutral-600">Expires at (optional)</span>
              <input
            ...
            <label className="flex flex-col gap-1 text-sm" htmlFor="members-invite-note">
              <span className="font-medium text-neutral-600">Note (optional)</span>
              <input
          ...
          <div className="rounded border border-neutral-200 bg-neutral-50 p-4 text-sm text-neutral-600">
            You can view members of {storeName}, but you do not have permission to manage invites.
          </div>
        )}
        ...
        <section className="flex flex-col gap-3 rounded border border-neutral-200 p-4">
          <div className="flex items-center justify-between">
            ...
            {invitesLoading ? <span className="text-xs text-neutral-500">Loading...</span> : null}
          </div>
          {!invites.length ? (
                ...
                <div key={invite.id} className="rounded border border-neutral-200 p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-neutral-500">Code: {invite.code || "-"}</span>
                      <span className="text-xs text-neutral-500">
                        Uses: {invite.used}/{invite.maxUses || "unlimited"}
                      </span>
                      <span className="text-xs text-neutral-500">
                        Flags: {invite.flags.length ? invite.flags.join(", ") : "-"}
                      </span>
                      <span className="text-xs text-neutral-500">Created: {formatIsoDatetime(invite.createdAt ?? null)}</span>
                      <span className="text-xs text-neutral-500">Expires: {formatIsoDatetime(invite.expiresAt ?? null)}</span>
                      <span className="text-xs text-neutral-500">Link: {invite.link}</span>
                      {invite.note ? <span className="text-xs text-neutral-500">Note: {invite.note}</span> : null}
                    </div>
                    {effectiveCanManage ? (
                      <button
                        type="button"
                      ...
                      >
                        {invite.status === "revoked" ? "Revoked" : "Revoke invite"}
                      </button>
                    ) : null}
                  </div>
                  <div className="mt-2 text-xs text-neutral-500">Status: {invite.status}</div>
                </div>
              ))}
            </div>
          )}
